...
export function harmonicMean(...params: unknown[]): number {
	...
	if (coeffs !== undefined) {
		return (
			coeffs.reduce((prev, curr) => prev + curr) /
            values.map((value, index) => 1 / (value * coeffs[index])).reduce((prev, curr) => prev + curr)			
		)
	}
	return values.length / values.map(value => 1 / value).reduce((prev, curr) => prev + curr)
}
 ...
 */
 ...
 export function quadraticMean(...params: unknown[]): number {
     const [values, coeffs] = paramsToValuesCoeffs(params)
     if (coeffs !== undefined) {
         return Math.sqrt(mean(values.map(value => value ** 2), coeffs.map(coeff => coeff ** 2)))
     }
