import { random, randomArray, randomInt, randomIntArray } from './random.ts';
...
Deno.test({
    ...
    fn: () => {
        ...
        //BigInt
        const [bigMin, bigMax] = [
            BigInt(Math.round(min)),
            BigInt(Math.round(max)) + 1n,
        ];
        const bigRandoms = new Array(5).fill(1n).map((_) =>
            randomInt(min, max)
        );
        for (const bigRand of bigRandoms) {
            if (bigRand < bigMin || bigRand > bigMax) {
                throw new Error(
                    `${bigRand} is out of range [${bigMin}, ${bigMax}]`,
                );
            }
        }
    },
...
Deno.test({
    ...
    fn: () => {
        ...
        if (randoms.length != length) {
            throw new Error(
                `Does not match length ${length} ${randoms.length} `,
            );
        }
    },
});
