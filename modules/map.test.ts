...
Deno.test({
    ...
    fn: () => {
        ...
        const initialBounds = [
            Math.min(...randomValues),
            Math.max(...randomValues),
        ]
            .sort((a, b) => a - b) as [number, number];
        ...
        assertEquals(
            ...
            randomValues.map((value) =>
                map(value, initialBounds, destinationBounds)
            )
                .map((value) => value.toFixed(4)),
            `Remap a random array of number from [${
                initialBounds.join(', ')
            }] to [${destinationBounds.join(', ')}]`,
        );
        //BigInt
        ...
        const bigRemappedValues = bigRandomValues.map((bigValue) =>
            (bigValue - bigInitialBounds[0]) *
                ...
                (bigInitialBounds[1] - bigInitialBounds[0]) +
            bigDestinationBounds[0]
        );
        assertEquals(
            bigRemappedValues,
