...
export function dichotomy(
    ...
    if (antiStackOverflow === moy) {
        throw new Error(`Unable to converge for ${expression} @ ${moy}`);
    }
    ...
    const compare = round(evaluate, precision) == expectedResult
        ? 0
        : round(evaluate, precision) > expectedResult
        ? 1
        : -1;
